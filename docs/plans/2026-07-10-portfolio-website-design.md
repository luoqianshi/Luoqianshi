...
2. 作品集区（Products）——主打项目卡片（缩略图 + 名称 + 简介 + 链接）
3. 其它作品区（Others）——截图表格窗格统一展示 + 文字列表（名称 + 一句话简介 + 链接）
4. 个人知识库区（Knowledge Base）——展示形式同其它作品区
5. 获奖情况区——奖项列表
6. 页脚——版权与更新时间

作品集数据通过 `category` 字段分类：`featured`（主打作品）、`others`（其它作品）、`knowledge`（个人知识库）。
